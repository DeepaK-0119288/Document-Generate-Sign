...
const Request: React.FC = () => {
  ...
  const handleUpload = async (fileList: File[]) => {
    ...
    try {
      ...
      //('Uploading documents:', { files: fileList.map(f => f.name), dataEntries });
  ...
  const handleDeleteDocument = async (document: Document) => {
    ...
    try {
      ...
      message.info(
        "Document deletion is not allowed while the request is in process."
      );
    }
  };
  ...
  const handleRejectDocument = async () => {
    ...
    try {
      ...
      setLoading(true);
      await requestClient.rejectDocument(
        id,
        selectedDocumentId,
        ...
        render: (_: any, record: Document) => (
          <Space>
            ...
            {(record.signStatus === signStatus.unsigned ||
              record.signStatus === signStatus.readForSign || 
              record.signStatus === signStatus.delegated) && (
              <Button
                icon={<EyeOutlined />}
                onClick={() => handlePreviewDocument(record)}
              >
            ...
            {isOfficer &&
              ...
              request?.status !== signStatusDisplay[signStatus.delegated] &&
              (record.signStatus === signStatus.unsigned ||
                record.signStatus === signStatus.readForSign) && (
                <Button
                  danger
